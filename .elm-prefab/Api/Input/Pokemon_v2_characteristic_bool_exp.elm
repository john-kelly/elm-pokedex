module Api.Input.Pokemon_v2_characteristic_bool_exp exposing (Pokemon_v2_characteristic_bool_exp, and, gene_mod_5, id, input, not, null, or, pokemon_v2_characteristicdescriptions, pokemon_v2_characteristicdescriptions_aggregate, pokemon_v2_stat, stat_id)

{-| 
## Creating an input

@docs pokemon_v2_characteristic_bool_exp, input

## Null values

@docs null

## Optional fields

@docs and, not, or, gene_mod_5, id, pokemon_v2_characteristicdescriptions, pokemon_v2_characteristicdescriptions_aggregate, pokemon_v2_stat, stat_id
-}


import Api.Input
import GraphQL.InputObject
import Json.Encode


type alias Pokemon_v2_characteristic_bool_exp =
    Api.Input.Pokemon_v2_characteristic_bool_exp


input : Pokemon_v2_characteristic_bool_exp
input =
    GraphQL.InputObject.inputObject "pokemon_v2_characteristic_bool_exp"


and :
    List Api.Input.Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
and newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "_and"
        "[pokemon_v2_characteristic_bool_exp!]"
        (Json.Encode.list GraphQL.InputObject.encode newArg_)
        inputObj_


not :
    Api.Input.Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
not newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "_not"
        "pokemon_v2_characteristic_bool_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


or :
    List Api.Input.Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
or newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "_or"
        "[pokemon_v2_characteristic_bool_exp!]"
        (Json.Encode.list GraphQL.InputObject.encode newArg_)
        inputObj_


gene_mod_5 :
    Api.Input.Int_comparison_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
gene_mod_5 newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "gene_mod_5"
        "Int_comparison_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


id :
    Api.Input.Int_comparison_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
id newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "id"
        "Int_comparison_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


pokemon_v2_characteristicdescriptions :
    Api.Input.Pokemon_v2_characteristicdescription_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
pokemon_v2_characteristicdescriptions newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "pokemon_v2_characteristicdescriptions"
        "pokemon_v2_characteristicdescription_bool_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


pokemon_v2_characteristicdescriptions_aggregate :
    Api.Input.Pokemon_v2_characteristicdescription_aggregate_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
pokemon_v2_characteristicdescriptions_aggregate newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "pokemon_v2_characteristicdescriptions_aggregate"
        "pokemon_v2_characteristicdescription_aggregate_bool_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


pokemon_v2_stat :
    Api.Input.Pokemon_v2_stat_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
pokemon_v2_stat newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "pokemon_v2_stat"
        "pokemon_v2_stat_bool_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


stat_id :
    Api.Input.Int_comparison_exp
    -> Pokemon_v2_characteristic_bool_exp
    -> Pokemon_v2_characteristic_bool_exp
stat_id newArg_ inputObj_ =
    GraphQL.InputObject.addField
        "stat_id"
        "Int_comparison_exp"
        (GraphQL.InputObject.encode newArg_)
        inputObj_


null :
    { and :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , not :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , or :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , gene_mod_5 :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , id :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , pokemon_v2_characteristicdescriptions :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , pokemon_v2_characteristicdescriptions_aggregate :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , pokemon_v2_stat :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    , stat_id :
        Pokemon_v2_characteristic_bool_exp -> Pokemon_v2_characteristic_bool_exp
    }
null =
    { and =
        \inputObj ->
            GraphQL.InputObject.addField
                "_and"
                "[pokemon_v2_characteristic_bool_exp!]"
                Json.Encode.null
                inputObj
    , not =
        \inputObj ->
            GraphQL.InputObject.addField
                "_not"
                "pokemon_v2_characteristic_bool_exp"
                Json.Encode.null
                inputObj
    , or =
        \inputObj ->
            GraphQL.InputObject.addField
                "_or"
                "[pokemon_v2_characteristic_bool_exp!]"
                Json.Encode.null
                inputObj
    , gene_mod_5 =
        \inputObj ->
            GraphQL.InputObject.addField
                "gene_mod_5"
                "Int_comparison_exp"
                Json.Encode.null
                inputObj
    , id =
        \inputObj ->
            GraphQL.InputObject.addField
                "id"
                "Int_comparison_exp"
                Json.Encode.null
                inputObj
    , pokemon_v2_characteristicdescriptions =
        \inputObj ->
            GraphQL.InputObject.addField
                "pokemon_v2_characteristicdescriptions"
                "pokemon_v2_characteristicdescription_bool_exp"
                Json.Encode.null
                inputObj
    , pokemon_v2_characteristicdescriptions_aggregate =
        \inputObj ->
            GraphQL.InputObject.addField
                "pokemon_v2_characteristicdescriptions_aggregate"
                "pokemon_v2_characteristicdescription_aggregate_bool_exp"
                Json.Encode.null
                inputObj
    , pokemon_v2_stat =
        \inputObj ->
            GraphQL.InputObject.addField
                "pokemon_v2_stat"
                "pokemon_v2_stat_bool_exp"
                Json.Encode.null
                inputObj
    , stat_id =
        \inputObj ->
            GraphQL.InputObject.addField
                "stat_id"
                "Int_comparison_exp"
                Json.Encode.null
                inputObj
    }